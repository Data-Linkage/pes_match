...
unique_matches = pd.read_csv(CHECKPOINT_PATH + 'Stage_2_Matchkey_Unique_Matches.csv', index_col=False)
...
crow_matches = crow_output_updater(output_df=crow_matches, id_column='puid', source_column='Source_Dataset',
                                   df1_name='_cen', df2_name='_pes', match_type='Stage_2_Conflicts')

# Save clerical matches
crow_matches.to_csv(CHECKPOINT_PATH + 'Stage_2_Matchkey_Conflict_Matches.csv', header=True, index=False)
...
all_matches = pd.concat([all_matches[OUTPUT_VARIABLES], stage_1_matches[OUTPUT_VARIABLES]])
...
assoc_uniques = collect_uniques(assoc_matches, id_1='puid_cen', id_2='puid_pes', match_type='Stage_2_Associative')
assoc_uniques.to_csv(CHECKPOINT_PATH + 'Stage_2_Associative_Unique_Matches.csv', header=True, index=False)
